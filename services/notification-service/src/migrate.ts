...
async function main(): Promise<void> {
  ...
  console.log(`[notification-service:migrate] ${direction} on ${location}`);

  const changed = await runMigrations(db, direction, migrations, { steps });
  console.log(
    changed.length
      ? `[notification-service:migrate] applied: ${changed.join(', ')}`
      : '[notification-service:migrate] nothing to do (up to date)',
  );
...
main()
  .then(() => process.exit(0))
  .catch((err: unknown) => {
    console.error('[notification-service:migrate] failed:', err);
    process.exit(1);
  });
