...
async function main(): Promise<void> {
  ...
  console.log(`[task-service:migrate] ${direction} on ${location}`);

  const changed = await runMigrations(db, direction, migrations, { steps });
  console.log(
    changed.length
      ? `[task-service:migrate] applied: ${changed.join(', ')}`
      : '[task-service:migrate] nothing to do (up to date)',
  );
...
main()
  .then(() => process.exit(0))
  .catch((err: unknown) => {
    console.error('[task-service:migrate] failed:', err);
    process.exit(1);
  });
