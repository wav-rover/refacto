...
async function main(): Promise<void> {
  ...
  console.log(`[auth-service:migrate] ${direction} on ${location}`);

  const changed = await runMigrations(db, direction, migrations, { steps });
  console.log(
    changed.length
      ? `[auth-service:migrate] applied: ${changed.join(", ")}`
      : "[auth-service:migrate] nothing to do (up to date)",
  );
...
main()
  .then(() => process.exit(0))
  .catch((err: unknown) => {
    console.error("[auth-service:migrate] failed:", err);
    process.exit(1);
  });
