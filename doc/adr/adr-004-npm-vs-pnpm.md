# ADR 004 – Stratégie workspaces : npm workspaces vs pnpm

- **Statut** : Accepté
- **Date** : 2026-05
- **Décideurs** : Équipe refacto
- **Contexte** : Début de la consolidation CI du mono-repo (app legacy + 5 packages sous `services/`).
...
- Les Dockerfiles par service restent, au départ, **hors workspace** (copie locale `package.json` + lock) tant qu’on n’a pas défini une stratégie de build racine — acceptable pour l’isolation des images.

### Actions de suivi

1. Ajouter `"workspaces": ["services/*"]` au `package.json` racine et fusionner les lockfiles en un `package-lock.json` racine.
2. Mettre à jour `.github/workflows/tests.yml` : une étape `npm ci`, supprimer la boucle sur `services/*/`.
3. Remplacer `scripts/ci-services.mjs` et `test:services` par des commandes workspace (`--workspaces`).
4. Harmoniser les versions dans les `package.json` des services (priorité : `task-service` vs les autres).
5. Documenter dans `README.md` : `npm ci` unique à la racine.
6. **Réévaluer pnpm** dans un ADR ultérieur si : temps CI > seuil convenu, besoin de `pnpm --filter` pour CI incrémental, ou politique d’isolation stricte imposée.
